                ...
                <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                  <div
                    className={cn(
                      ...
                      SUBJECT_GRADIENTS[s.ci % SUBJECT_GRADIENTS.length],
                    )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass rounded-xl p-4 text-center">
        {streak === 0 ? (
          <p className="text-sm text-muted-foreground">Open the app daily to build your streak</p>
        ) : streak >= bestStreak ? (
          <>
            <p className="text-sm font-medium">You are on your best streak ever!</p>
            <p className="text-xs text-muted-foreground mt-1">Keep going — each day counts</p>
          </>
        ) : (
          <>
            <p className="text-sm font-medium">
              Next goal: <span className="text-primary-glow">{bestStreak} days</span>
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {bestStreak - streak} more day{bestStreak - streak !== 1 ? "s" : ""} to beat your
              record
            </p>
          </>
        )}
...
function FocusScoreDetail({
  ...
  const breakdown = [
    {
      label: "Study time",
      pct: Math.round((studyH / total) * 100),
      color: "from-indigo-500 to-purple-500",
    },
    {
      label: "Class time",
      pct: Math.round((classH / total) * 100),
...
const STAT_CONFIGS = [
  {
    ...
    dialogTitle: "Study Hours",
    dialogDesc: "Daily breakdown and subject distribution",
  },
  {
    label: "Streak",
    icon: Flame,
    accent: "from-amber-500 to-rose-500",
    dialogTitle: "Study Streak",
    dialogDesc: "Consistency calendar and milestones",
  },
  {
    label: "Focus score",
    icon: Target,
    accent: "from-blue-500 to-cyan-500",
    dialogTitle: "Focus Score",
    dialogDesc: "Study vs class time balance",
  },
  {
    label: "Sessions this week",
    icon: Brain,
    accent: "from-violet-500 to-fuchsia-500",
    dialogTitle: "Study Sessions",
    dialogDesc: "Session count per subject",
  },
] as const;
...
export function StatsGrid() {
  ...
  // Real calendar-day streak (persists across logouts, records today on mount)
  const { streak, bestStreak, activeDaysThisWeek, streakGrid } = useStreak(user?.id);
  ...
  const computed = useMemo((): Computed | null => {
    ...
    const focusScore =
      studyMins + classMins > 0 ? Math.round((studyMins / (studyMins + classMins)) * 100) : 0;
    ...
    const dailyHours: DailyRow[] = DAY_LABELS.map((d, i) => {
      ...
      return {
        d,
        study: r1(
          de.filter((e) => e.type === "study").reduce((s, e) => s + e.end - e.start, 0) / 60,
        ),
        class: r1(
          de.filter((e) => e.type === "class").reduce((s, e) => s + e.end - e.start, 0) / 60,
        ),
      };
    });
    ...
    return {
      ...
      totalEvents,
      focusScore,
      // Use real streak data from useStreak (passed in from outer scope)
      streak,
      bestStreak,
      dailyHours,
      subjectStudyHours,
  ...
  }, [events, subjects, hasData, streak, bestStreak, activeDaysThisWeek, streakGrid]);

  const cards = computed
    ? [
        {
          value: fmtH(computed.studyMins),
          delta: `of ${fmtH(computed.totalMins)} total scheduled`,
        },
        {
          value: `${computed.streak} day${computed.streak !== 1 ? "s" : ""}`,
          delta:
            computed.activeDays > 0
              ? `${computed.activeDays} of 7 this week · best ${computed.bestStreak}d`
              : `Best streak: ${computed.bestStreak} day${computed.bestStreak !== 1 ? "s" : ""}`,
        },
        {
          value: String(computed.focusScore),
          delta: `${fmtH(computed.studyMins)} study / ${fmtH(computed.classMins)} class`,
        },
        {
          value: `${computed.studySessions} / ${computed.totalEvents}`,
          delta: `${computed.totalEvents > 0 ? Math.round((computed.studySessions / computed.totalEvents) * 100) : 0}% are study blocks`,
        },
      ]
    : [
        { value: "—", delta: "No schedule yet" },
        {
          ...
          delta: streak > 0 ? `Best: ${bestStreak}d` : "Open app daily to build streak",
        },
  ...
  const DIALOG_STYLE: React.CSSProperties = {
    ...
    border: "1px solid var(--border)",
    boxShadow: "0 1px 0 oklch(1 0 0 / 0.14) inset, 0 32px 80px -16px oklch(0.04 0.02 275 / 0.88)",
    borderRadius: "20px",
  };
        ...
        {STAT_CONFIGS.map((cfg, i) => (
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse 70% 35% at 20% 0%, oklch(1 0 0 / 0.06) 0%, transparent 55%)",
              }}
            />
      ...
      <Dialog open={open !== null} onOpenChange={(v) => !v && setOpen(null)}>
        {activeConfig && computed && (
          <DialogContent className="max-w-lg border-0 p-6" style={DIALOG_STYLE}>
            ...
            {open === 0 && (
              <StudyHoursDetail
                daily={computed.dailyHours}
                subjHours={computed.subjectStudyHours}
              />
            )}
            {open === 1 && (
              <StreakDetail
            ...
            {open === 2 && (
              <FocusScoreDetail
                ...
                daily={computed.dailyHours}
                studyMins={computed.studyMins}
                classMins={computed.classMins}
              />
            )}
